...
function konfirmasi_token(id) {
	...
	if (token_asli != token_input) {
		alert("Gagal Memasukan Token! Masukan kembali dengan benar.");
		return false;
	} else {
		alert("Berhasil Memasukan Token! Selamat Ujian.");
...
function m_soal_h(id) {
	if (confirm('Anda yakin ingin menghapus soal?')) {
		$.ajax({
			type: "GET",
...
function m_guru_jurusan(id) {
	$.ajax({
		type: "GET",
		url: base_url+"adm/m_guru/ambil_jurusan/"+id,
		success: function(data) {
			if (data.status == "ok") {
				var jml_data	= Object.keys(data.data).length;
				var hate 	= '<div class="modal fade" id="m_siswa_matkul" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 id="myModalLabel">Setting jurusan</h4></div><div class="modal-body"><form name="f_siswa_matkul" id="f_siswa_matkul" method="post" onsubmit="return    m_guru_jurusan_s();"><input type="hidden" name="id_mhs" id="id_mhs" value="'+id+'"><div id="konfirmasi"></div>';

				if (jml_data > 0) {
					$.each(data.data, function(i, item) {
						if (item.ok == "1") {
							hate += '<label><input type="checkbox" value="'+item.id+'" name="id_jurusan_'+item.id+'" checked> &nbsp;'+item.nama+'</label> &nbsp;&nbsp <br> ';
						} else {
							hate += '<label><input type="checkbox" value="'+item.id+'" name="id_jurusan_'+item.id+'"> &nbsp;'+item.nama+'</label> &nbsp;&nbsp; <br>';
						}
					});
				} else {
					hate += 'Belum ada data..';
...
function m_guru_jurusan_s() {
	var f_asal	= $("#f_siswa_matkul");
	var form	= getFormData(f_asal);
	$.ajax({
		type: "POST",
		url: base_url+"adm/m_guru/simpan_jurusan",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function(response) {
