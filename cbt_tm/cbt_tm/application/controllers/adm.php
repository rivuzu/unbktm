...
if ($sess_level == "siswa") {
	...
	public function m_siswa() {
		...
		if($a['sess_level'] == "admin"){
			// $a['d'] =
			$a['p_jurusan'] = obj_to_array($this->db->query("SELECT * FROM tm_jurusan")->result(), "id,nama");
			// $a['d'] = "m_jurusan";
		}
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM tm_siswa WHERE id = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE tm_siswa SET nama = '".bersih($p,"nama")."', nim = '".bersih($p,"nim")."', id_jurusan = '".bersih($p,"jurusan")."'	WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO tm_siswa VALUES (null, '".bersih($p,"nama")."','".bersih($p,"nim")."','".bersih($p,"jurusan")."')");
			}
			$ret_arr['status'] 	= "ok";
			...
			$this->db->query("DELETE FROM tm_siswa WHERE id = '".$uri4."'");
			$this->db->query("DELETE FROM tm_admin WHERE level = 'siswa' AND kon_id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
		} else if ($uri3 == "user") {
			$det_user = $this->db->query("SELECT id, nim FROM tm_siswa WHERE id = '$uri4'")->row();

			if (!empty($det_user)) {
				$q_cek_username = $this->db->query("SELECT id FROM tm_admin WHERE username = '".$det_user->nim."' AND level = 'siswa'")->num_rows();

				if ($q_cek_username < 1) {

					$this->db->query("INSERT INTO tm_admin VALUES (null, '".$det_user->nim."', md5('".$det_user->nim."'), 'siswa', '".$det_user->id."')");
					$ret_arr['status'] 	= "ok";
			...
			$this->db->query("UPDATE tm_admin SET password = md5('".$det_user->nim."') WHERE level = 'siswa' AND kon_id = '".$det_user->id."'");
			...
			$matkul = $this->db->query("SELECT tm_mapel.*,
										(SELECT COUNT(id) FROM tr_siswa_mapel WHERE id_siswa = ".$uri4." AND id_mapel = tm_mapel.id) AS ok
										FROM tm_mapel
										")->result();
			$ret_arr['status'] = "ok";
			$ret_arr['data'] = $matkul;
			...
			$ket 	= "";
			//echo var_dump($p);
			$ambil_matkul = $this->db->query("SELECT id FROM tm_mapel ORDER BY id ASC")->result();
	        ...
	        $d_total_row = $this->db->query("SELECT id FROM tm_siswa a WHERE a.nama LIKE '%".$search['value']."%'")->num_rows();

	        $q_datanya = $this->db->query("SELECT a.*,b.nama as jurusan,
											(SELECT COUNT(id) FROM tm_admin WHERE level = 'siswa' AND kon_id = a.id) AS ada
											FROM tm_siswa a
											INNER JOIN tm_jurusan b ON a.id_jurusan = b.id
	...
	public function m_guru() {
		...
		$jeson = array();
		/*
		$a['data'] = $this->db->query("SELECT tm_guru.*,
									(SELECT COUNT(id) FROM tm_admin WHERE level = 'guru' AND kon_id = tm_guru.id) AS ada
									FROM tm_guru")->result();
		*/

		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM tm_guru WHERE id = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE tm_guru SET nama = '".bersih($p,"nama")."', nip = '".bersih($p,"nip")."' WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO tm_guru VALUES (null, '".bersih($p,"nip")."', '".bersih($p,"nama")."')");
			}
			...
			$this->db->query("DELETE FROM tm_guru WHERE id = '".$uri4."'");
			$this->db->query("DELETE FROM tm_admin WHERE level = 'guru' AND kon_id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
		} else if ($uri3 == "user") {
			$det_user = $this->db->query("SELECT id, nip FROM tm_guru WHERE id = '$uri4'")->row();

			if (!empty($det_user)) {
				$q_cek_username = $this->db->query("SELECT id FROM tm_admin WHERE username = '".$det_user->nip."' AND level = 'guru'")->num_rows();

				if ($q_cek_username < 1) {

					$this->db->query("INSERT INTO tm_admin VALUES (null, '".$det_user->nip."', md5('".$det_user->nip."'), 'guru', '".$det_user->id."')");
					$ret_arr['status'] 	= "ok";
					$ret_arr['caption']	= "tambah user sukses";
					j($ret_arr);
				} else {
			...
			$this->db->query("UPDATE tm_admin SET password = md5('".$det_user->nip."') WHERE level = 'guru' AND kon_id = '".$det_user->id."'");
			...
			$matkul = $this->db->query("SELECT tm_mapel.*,
										(SELECT COUNT(id) FROM tm_guru_mapel WHERE id_guru = ".$uri4." AND id_mapel = tm_mapel.id) AS ok
										FROM tm_mapel
										")->result();
			$ret_arr['status'] = "ok";
			$ret_arr['data'] = $matkul;
			...
			if (!empty($ambil_matkul)) {
				foreach ($ambil_matkul as $a) {
					...
					if (!empty($p->$p_sub)) {
						...
						$cek_sudah_ada = $this->db->query("SELECT id FROM tm_guru_mapel WHERE  id_guru = '".$p->id_mhs."' AND id_mapel = '".$a->id."'")->num_rows();

						if ($cek_sudah_ada < 1) {
							$this->db->query("INSERT INTO tm_guru_mapel VALUES (null, '".$p->id_mhs."', '".$a->id."')");
						} else {
							$this->db->query("UPDATE tm_guru_mapel SET id_mapel = '".$p->$p_sub."' WHERE id_guru = '".$p->id_mhs."' AND id_mapel = '".$a->id."'");
						}
					} else {
						//echo "0<br>";
						$this->db->query("DELETE FROM tm_guru_mapel WHERE id_guru = '".$p->id_mhs."' AND id_mapel = '".$a->id."'");
					}
				}
			}
			$ret_arr['status'] 	= "ok";
			...
			j($ret_arr);
			exit();
		}else if ($uri3 == "ambil_jurusan") {
			$matkul = $this->db->query("SELECT tm_jurusan.*,
										(SELECT COUNT(id) FROM tm_guru_jurusan WHERE id_guru = ".$uri4." AND id_jurusan = tm_jurusan.id) AS ok
										FROM tm_jurusan
										")->result();
			$ret_arr['status'] = "ok";
			$ret_arr['data'] = $matkul;
			j($ret_arr);
			exit;
		} else if ($uri3 == "simpan_jurusan") {
			$ket 	= "";
			//echo var_dump($p);
			$ambil_matkul = $this->db->query("SELECT id FROM tm_jurusan ORDER BY id ASC")->result();
			if (!empty($ambil_matkul)) {
				foreach ($ambil_matkul as $a) {
					$p_sub = "id_jurusan_".$a->id;
					if (!empty($p->$p_sub)) {

						$cek_sudah_ada = $this->db->query("SELECT id FROM tm_guru_jurusan WHERE  id_guru = '".$p->id_mhs."' AND id_jurusan = '".$a->id."'")->num_rows();

						if ($cek_sudah_ada < 1) {
							$this->db->query("INSERT INTO tm_guru_jurusan VALUES (null, '".$p->id_mhs."', '".$a->id."')");
						} else {
							$this->db->query("UPDATE tm_guru_jurusan SET id_jurusan = '".$p->$p_sub."' WHERE id_guru = '".$p->id_mhs."' AND id_jurusan = '".$a->id."'");
						}
					} else {
						//echo "0<br>";
						$this->db->query("DELETE FROM tm_guru_jurusan WHERE id_guru = '".$p->id_mhs."' AND id_jurusan = '".$a->id."'");
					}
				}
			}
			$ret_arr['status'] 	= "ok";
	        ...
	        $d_total_row = $this->db->query("SELECT id FROM tm_guru a WHERE a.nama LIKE '%".$search['value']."%'")->num_rows();

	        $q_datanya = $this->db->query("SELECT a.*,
											(SELECT COUNT(id) FROM tm_admin WHERE level = 'guru' AND kon_id = a.id) AS ada
											FROM tm_guru a
	                                        WHERE a.nama LIKE '%".$search['value']."%' ORDER BY a.id DESC LIMIT ".$start.", ".$length."")->result_array();
	        $data = array();
	        $no = ($start+1);
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[3] = '<div class="btn-group">
                          <a href="#" onclick="return m_guru_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 5px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_guru_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-remove" style="margin-left: 5px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                          <a href="#" onclick="return m_guru_matkul('.$d['id'].');" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-th-list" style="margin-left: 5px; color: #fff"></i> &nbsp;&nbsp;MaPel Diambil</a>
													<a href="#" onclick="return m_guru_jurusan('.$d['id'].');" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-th-list" style="margin-left: 5px; color: #fff"></i> &nbsp;&nbsp;Jurusan Diambil</a>
                         ';
	...
	public function m_mapel() {
		...
		//return as json
		$jeson = array();
		$a['data'] = $this->db->query("SELECT tm_mapel.* FROM tm_mapel")->result();
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM tm_mapel WHERE id = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE tm_mapel SET nama = '".bersih($p,"nama")."'
								WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO tm_mapel VALUES (null, '".bersih($p,"nama")."')");
			}
			...
			$this->db->query("DELETE FROM tm_mapel WHERE id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
	        ...
	        $d_total_row = $this->db->query("SELECT id FROM tm_mapel a WHERE a.nama LIKE '%".$search['value']."%'")->num_rows();
	...
	public function m_soal() {
		...
		if ($a['sess_level'] == "guru") {
			$a['p_guru'] = obj_to_array($this->db->query("SELECT * FROM tm_guru WHERE id = '".$a['sess_konid']."'")->result(), "id,nama");
			// $a['p_jurusan'] = obj_to_array($this->db->query("SELECT
			// 								b.id, b.nama
			// 								FROM tm_guru_jurusan a
			// 								INNER JOIN tm_jurusan b ON a.id_jurusan = b.id
			// 								WHERE a.id_guru = '".$a['sess_konid']."'")->result(), "id_jurusan,jurusan");
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT
											b.id, b.nama
											FROM tm_guru_mapel a
											INNER JOIN tm_mapel b ON a.id_mapel = b.id
											WHERE a.id_guru = '".$a['sess_konid']."'")->result(), "id,nama");
		} else {
			$a['p_guru'] = obj_to_array($this->db->query("SELECT * FROM tm_guru")->result(), "id,nama");
			// $a['p_mapel'] = obj_to_array($this->db->query("SELECT
			// 								b.id, b.nama
			// 								FROM tm_guru_jurusan a
			// 								INNER JOIN tm_jurusan b ON a.id_mapel = b.id")->result(), "id,nama");
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT
											b.id, b.nama
											FROM tm_guru_mapel a
											INNER JOIN tm_mapel b ON a.id_mapel = b.id")->result(), "id,nama");
		}

		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM tm_soal WHERE id = '$uri4' ORDER BY id DESC")->row();
			j($a);
			exit();
		} else if ($uri3 == "import") {
			$a['p']	= "f_soal_import";
		} else if ($uri3 == "hapus_gambar") {
			$nama_gambar = $this->db->query("SELECT file FROM tm_soal WHERE id = '".$uri5."'")->row();
			$this->db->query("UPDATE tm_soal SET file = '', tipe_file = '' WHERE id = '".$uri5."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			redirect('adm/m_soal/pilih_mapel/'.$uri4);
		} else if ($uri3 == "pilih_mapel") {
			if ($a['sess_level'] == "guru") {
				$a['data'] = $this->db->query("SELECT tm_soal.*, tm_guru.nama AS nama_guru FROM tm_soal INNER JOIN tm_guru ON tm_soal.id_guru = tm_guru.id WHERE tm_soal.id_guru = '".$a['sess_konid']."' AND tm_soal.id_mapel = '$uri4' ORDER BY id DESC")->result();
			} else {
				$a['data'] = $this->db->query("SELECT tm_soal.*, tm_guru.nama AS nama_guru FROM tm_soal INNER JOIN tm_guru ON tm_soal.id_guru = tm_guru.id WHERE tm_soal.id_mapel = '$uri4' ORDER BY id DESC")->result();
			}
			...
			if ($__mode == "edit") {
				...
				$this->db->update("tm_soal", $pdata);
				$__id_soal = $p['id'];
			} else {
				$this->db->insert("tm_soal", $pdata);
				$get_id_akhir = $this->db->query("SELECT MAX(id) maks FROM tm_soal LIMIT 1")->row_array();
			...
			//ambil data awal
			$get_opsi_awal = $this->db->query("SELECT opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM tm_soal WHERE id = '".$__id_soal."'")->row_array();
			...
			$this->db->update("tm_soal", $data_simpan);
			...
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT b.id, b.nama FROM tm_guru_mapel a INNER JOIN tm_mapel b ON a.id_mapel = b.id $id_guru")->result(),"id,nama");

			if ($uri4 == 0) {
				$a['d'] = array("mode"=>"add","id"=>"0","id_guru"=>$id_guru,"id_mapel"=>"","bobot"=>"1","file"=>"","soal"=>"","opsi_a"=>"#####","opsi_b"=>"#####","opsi_c"=>"#####","opsi_d"=>"#####","opsi_e"=>"#####","jawaban"=>"","tgl_input"=>"");
			} else {
				$a['d'] = $this->db->query("SELECT tm_soal.*, 'edit' AS mode FROM tm_soal WHERE id = '$uri4'")->row_array();
			...
			$nama_gambar = $this->db->query("SELECT id_mapel, file, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM tm_soal WHERE id = '".$uri4."'")->row();
			...
			$this->db->query("DELETE FROM tm_soal WHERE id = '".$uri4."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			...
			if ($a['sess_level'] == "admin") {
				$data = $this->db->query("SELECT * FROM tm_soal")->result();
			} else {
				$data = $this->db->query("SELECT * FROM tm_soal WHERE id_guru = '".$a['sess_konid']."'")->result();
			}

			$mapel = $this->db->query("SELECT nama FROM tm_mapel WHERE id = '".$uri4."'")->row();
			if (!empty($data)) {
		        ...
		        $d_total_row = $this->db->query("SELECT a.*
												FROM tm_soal a
												INNER JOIN tm_guru b ON a.id_guru = b.id
												INNER JOIN tm_mapel c ON a.id_mapel = c.id
	...
	public function m_ujian() {
		...
		//$a['data'] = $this->db->query("SELECT tm_ujian.*, tm_mapel.nama AS mapel FROM tm_ujian INNER JOIN tm_mapel ON tm_ujian.id_mapel = tm_mapel.id WHERE tm_ujian.id_guru = '".$a['sess_konid']."'")->result();

		$a['pola_tes'] = array(""=>"Pengacakan Soal", "acak"=>"Soal Diacak", "set"=>"Soal Diurutkan");
		$a['p_jurusan'] = obj_to_array($this->db->query("SELECT * FROM tm_jurusan WHERE id IN (SELECT id_jurusan FROM tm_guru_jurusan WHERE id_guru = '".$a['sess_konid']."')")->result(), "id,nama");
		$a['p_mapel'] = obj_to_array($this->db->query("SELECT * FROM tm_mapel WHERE id IN (SELECT id_mapel FROM tm_guru_mapel WHERE id_guru = '".$a['sess_konid']."')")->result(), "id,nama");

		if ($uri3 == "det") {
			$are = array();

			$a = $this->db->query("SELECT * FROM tm_ujian WHERE id = '$uri4'")->row();
			...
			if ($p->id != 0) {
				$this->db->query("UPDATE tm_ujian SET id_mapel = '".bersih($p,"mapel")."', id_jurusan = '".bersih($p,"jurusan")."',
				...
				$this->db->query("INSERT INTO tm_ujian VALUES (null, '".$a['sess_konid']."', '".bersih($p,"mapel")."', '".bersih($p,"jurusan")."',
								'".bersih($p,"nama_ujian")."', '".bersih($p,"jumlah_soal")."', '".bersih($p,"waktu")."', '".bersih($p,"acak")."',
								'', '".bersih($p,"tgl_mulai")." ".bersih($p,"wkt_mulai")."', '$token')");
			}
			...
			$this->db->query("DELETE FROM tm_ujian WHERE id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
		} else if ($uri3 == "jumlah_soal") {
			$ambil_data = $this->db->query("SELECT id FROM tm_soal WHERE id_mapel = '$uri4' AND id_guru = '".$a['sess_konid']."'")->num_rows();
			...
			exit();
		        ...
		        $q_datanya = $this->db->query("SELECT a.*, b.nama AS mapel,c.nama AS jurusan
												FROM tm_ujian a
									        	INNER JOIN tm_mapel b ON a.id_mapel = b.id
														INNER JOIN tm_jurusan c ON a.id_jurusan = c.id
									        	WHERE a.id_guru = '".$a['sess_konid']."'
							                    AND (a.nama_ujian LIKE '%".$search['value']."%'
												OR b.nama LIKE '%".$search['value']."%')
		                                        ORDER BY a.id DESC LIMIT ".$start.", ".$length."")->result_array();
			...
			$this->db->query("UPDATE tm_ujian SET token = '$token' WHERE id = '$uri4'");
	...
	public function h_ujian() {
		...
		$a['p_mapel'] = obj_to_array($this->db->query("SELECT * FROM tm_mapel")->result(), "id,nama");

		if ($uri3 == "det") {
			$a['detil_tes'] = $this->db->query("SELECT tm_mapel.nama AS namaMapel, tm_guru.nama AS nama_guru,
												tm_ujian.*
												FROM tm_ujian
												INNER JOIN tm_mapel ON tm_ujian.id_mapel = tm_mapel.id
												INNER JOIN tm_guru ON tm_ujian.id_guru = tm_guru.id
												WHERE tm_ujian.id = '$uri4'")->row();
			$a['statistik'] = $this->db->query("SELECT MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_
											FROM tm_ikut_ujian
											WHERE tm_ikut_ujian.id_tes = '$uri4'")->row();
	        ...
	        $d_total_row = $this->db->query("
	        	SELECT a.id
				FROM tm_ikut_ujian a
				INNER JOIN tm_siswa b ON a.id_user = b.id
				WHERE a.id_tes = '$uri4'
			...
			$this->db->query("DELETE FROM tm_ikut_ujian WHERE id = '$uri4'");
	        ...
	        $d_total_row = $this->db->query("SELECT a.id FROM tm_ujian a
	        	INNER JOIN tm_mapel b ON a.id_mapel = b.id
	        	INNER JOIN tm_guru c ON a.id_guru = c.id
	            WHERE (a.nama_ujian LIKE '%".$search['value']."%' OR b.nama LIKE '%".$search['value']."%' OR c.nama LIKE '%".$search['value']."%') ".$wh_1."")->num_rows();
	    	//echo $this->db->last_query();

	        $q_datanya = $this->db->query("SELECT a.*, b.nama AS mapel, c.nama AS nama_guru FROM tm_ujian a
	        	INNER JOIN tm_mapel b ON a.id_mapel = b.id
	        	INNER JOIN tm_guru c ON a.id_guru = c.id
	...
	public function hasil_ujian_cetak() {
		...
		$a['detil_tes'] = $this->db->query("SELECT tm_mapel.nama AS namaMapel, tm_guru.nama AS nama_guru,
												...
												INNER JOIN tm_mapel ON tm_ujian.id_mapel = tm_mapel.id
												INNER JOIN tm_guru ON tm_ujian.id_guru = tm_guru.id
												WHERE tm_ujian.id = '$uri3'")->row();

		$a['statistik'] = $this->db->query("SELECT MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_
										FROM tm_ikut_ujian
										WHERE tm_ikut_ujian.id_tes = '$uri3'")->row();
		$a['hasil'] = $this->db->query("SELECT tm_siswa.nama, tm_ikut_ujian.nilai, tm_ikut_ujian.jml_benar, tm_ikut_ujian.nilai_bobot
										FROM tm_ikut_ujian
										INNER JOIN tm_siswa ON tm_ikut_ujian.id_user = tm_siswa.id
										WHERE tm_ikut_ujian.id_tes = '$uri3'")->result();
	...
	public function ikuti_ujian() {
		...
		$a['data'] = $this->db->query("SELECT
									...
									INNER JOIN tm_mapel b ON a.id_mapel = b.id
									INNER JOIN tm_guru c ON a.id_guru = c.id
									LEFT JOIN tm_ikut_ujian d ON CONCAT('".$a['sess_konid']."',a.id) = CONCAT(d.id_user,d.id_tes)
	...
	public function ikut_ujian() {
		...
		//var post from json
		$p = json_decode(file_get_contents('php://input'));
		$a['detil_user'] = $this->db->query("SELECT * FROM tm_siswa WHERE id = '".$a['sess_konid']."'")->row();
		if ($uri3 == "simpan_satu") {
			$p			= json_decode(file_get_contents('php://input'));
			...
			$this->db->query("UPDATE tm_ikut_ujian SET list_jawaban = '".$update_."' WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
			//echo $this->db->last_query();

			$q_ret_urn 	= $this->db->query("SELECT list_jawaban FROM tm_ikut_ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
			...
			for ($i = 1; $i < $p->jml_soal; $i++) {
				...
				$_tidsoal 	= "id_soal_".$i;
				$jawaban_ 	= empty($p->$_tjawab) ? "" : $p->$_tjawab;
				$cek_jwb 	= $this->db->query("SELECT bobot, jawaban FROM tm_soal WHERE id = '".$p->$_tidsoal."'")->row();
				//untuknilai bobot
				...
				if ($cek_jwb->jawaban == $jawaban_) {
					...
					$q_update_jwb = "UPDATE tm_soal SET jml_benar = jml_benar + 1 WHERE id = '".$p->$_tidsoal."'";
				} else {
					//jika jawaban salah
					$array_nilai[$bobotnya] = empty($array_nilai[$bobotnya]) ? 0 : $array_nilai[$bobotnya] + 0;
					$q_update_jwb = "UPDATE tm_soal SET jml_salah = jml_salah + 1 WHERE id = '".$p->$_tidsoal."'";
				}
			...
			exit;
			*/
			$this->db->query("UPDATE tm_ikut_ujian SET jml_benar = ".$jumlah_benar.", nilai_bobot = ".$nilai_bobot.", nilai = '".$nilai."', list_jawaban = '".$update_."', status = 'N' WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
			$a['status'] = "ok";
			j($a);
			exit;
		} else if ($uri3 == "token") {
			$a['du'] = $this->db->query("SELECT a.id, a.tgl_mulai,
										a.token, a.nama_ujian, a.jumlah_soal, a.waktu,
										b.nama nmguru, c.nama nmmapel FROM tm_ujian a
										INNER JOIN tm_guru b ON a.id_guru = b.id
										INNER JOIN tm_mapel c ON a.id_mapel = c.id
										WHERE a.id = '$uri4'")->row_array();
			$a['dp'] = $this->db->query("SELEcT * FROM tm_siswa WHERE id = '".$a['sess_konid']."'")->row_array();
			...
			$cek_sdh_selesai= $this->db->query("SELECT id FROM tm_ikut_ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."' AND status = 'N'")->num_rows();

			//sekalian validasi waktu sudah berlalu...
			if ($cek_sdh_selesai < 1) {
				//ini jika ujian belum tercatat, belum ikut
				//ambil detil soal
				$cek_detil_tes = $this->db->query("SELECT * FROM tm_ujian WHERE id = '$uri4'")->row();
				$q_cek_sdh_ujian= $this->db->query("SELECT id FROM tm_ikut_ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
				...
				if ($cek_sdh_ujian < 1)	{
					$soal_urut_ok = array();
					$q_soal			= $this->db->query("SELECT id, file, tipe_file, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, '' AS jawaban FROM tm_soal WHERE id_mapel = '".$cek_detil_tes->id_mapel."' AND id_guru = '".$cek_detil_tes->id_guru."' ".$acakan." LIMIT ".$cek_detil_tes->jumlah_soal)->result();
					$i = 0;
					foreach ($q_soal as $s) {
					...
					$this->db->query("INSERT INTO tm_ikut_ujian VALUES (null, '$uri4', '".$a['sess_konid']."', '$list_id_soal', '$list_jw_soal', 0, 0, 0, '$time_mulai', ADDTIME('$time_mulai', '$waktu_selesai'), 'Y')");

					$detil_tes = $this->db->query("SELECT * FROM tm_ikut_ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'")->row();

					$soal_urut_ok= $soal_urut_ok;
				} else {
					$q_ambil_soal 	= $this->db->query("SELECT * FROM tm_ikut_ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'")->row();
					...
					for ($i = 0; $i < sizeof($urut_soal); $i++) {
						...
						$ambil_soal = $this->db->query("SELECT *, $pc_urut_soal1 AS jawaban FROM tm_soal WHERE id = '".$pc_urut_soal[0]."'")->row();
				        ...
				        for ($j = 0; $j < $this->config->item('jml_opsi'); $j++) {
				            ...
				            $html .= '<div class="cek">
																<label for="opsi_'.strtoupper($this->opsi[$j]).'_'.$d->id.'">
																	<div class="huruf_opsi" style="display:inline-block;">
																		<input type="radio" id="opsi_'.strtoupper($this->opsi[$j]).'_'.$d->id.'" name="opsi_'.$no.'" value="'.strtoupper($this->opsi[$j]).'" '.$checked.'> '.strtoupper($this->opsi[$j]).'
																	</div>
																	<div class="opsi_s" style="display:inline-block; margin-left:10px;">
																		<p>'.$tampil_media_opsi.'</p> <p>'.$pc_pilihan_opsi[1].'</p>
																	</div>
																</label>
															</div>
											';
	...
	public function jvs() {
		...
		$data_soal 		= $this->db->query("SELECT id, gambar, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM tm_soal ORDER BY RAND()")->result();
	...
	public function rubah_password() {
		...
		if ($uri3 == "simpan") {
			...
			$p2_md5 = md5($p->p2);
			$p3_md5 = md5($p->p3);
			$cek_pass_lama = $this->db->query("SELECT password FROM tm_admin WHERE id = '".$a['sess_admin_id']."'")->row();
			if ($cek_pass_lama->password != $p1_md5) {
				...
				$this->db->query("UPDATE tm_admin SET password = '".$p3_md5."' WHERE id = '".$a['sess_admin_id']."'");
				$ret['status'] = "ok";
			...
			$data = $this->db->query("SELECT id, kon_id, level, username FROM tm_admin WHERE id = '".$a['sess_admin_id']."'")->row();
			j($data);
			exit;
		}
	}
	public function sudah_selesai_ujian() {
		...
		$q_nilai = $this->db->query("SELECT nilai, tgl_selesai FROM tm_ikut_ujian WHERE id_tes = $uri3 AND id_user = '".$a['sess_konid']."' AND status = 'N'")->row();
		if (empty($q_nilai)) {
	...
	public function act_login() {
		...
		$q_data		= $this->db->query("SELECT * FROM tm_admin WHERE username = '".$username."' AND password = '$password2'");
		...
		if ($j_data === 1) {
			...
			if ($a_data->level == "siswa") {
				$det_user = $this->db->query("SELECT nama FROM tm_siswa WHERE id = '".$a_data->kon_id."'")->row();
				if (!empty($det_user)) {
					$sess_nama_user = $det_user->nama;
				}
			} else if ($a_data->level == "guru") {
				$det_user = $this->db->query("SELECT nama FROM tm_guru WHERE id = '".$a_data->kon_id."'")->row();
				if (!empty($det_user)) {
					$sess_nama_user = $det_user->nama;
				}
			} else {
